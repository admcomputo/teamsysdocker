...
export const PublicPortafolioPage = () => {
  const { textoUrl } = useParams<{ textoUrl: string }>();
  ...
  useEffect(() => {
    const fetchPublicPortafolio = async () => {
      try {
        setLoading(true);
        setError(null);
        ...
        if (!profileRes.ok) {
          throw new Error("No se pudo cargar el perfil público del usuario.");
        }

        const profileData = await profileRes.json();
        const experiences = expRes.ok ? await expRes.json() : [];
        const proyectos = projRes.ok ? await projRes.json() : [];
        const habilidadesTecnicas = techRes.ok ? await techRes.json() : [];
        const habilidadesBlandas = softRes.ok ? await softRes.json() : [];
        const formacionesAcademica = eduRes.ok ? await eduRes.json() : [];

        // Mapeamos los datos al formato unificado de portafolio
        const portafolioMapeado: PortafolioCompleto = {
          ...
          enlacePublico: null,
          experienciasLaborales: experiences || [],
          formacionesAcademica: formacionesAcademica || [],
        ...
        setError(err instanceof Error ? err.message : "Error al cargar portafolio público.");
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="flex h-[60vh] items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-text-secondary">Cargando portafolio público...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="mx-auto max-w-xl text-center py-16 px-4">
        <div className="rounded-3xl border border-red-500/20 bg-red-950/20 p-8 backdrop-blur-md">
          ...
          <h2 className="mt-4 text-2xl font-bold text-text-primary">Error al cargar portafolio público</h2>
          <p className="mt-2 text-text-secondary">{error || "El enlace de este portafolio no es válido o ha expirado."}</p>
          <button
            onClick={() => navigate("/")}
    ...
    <div className="relative min-h-[calc(100vh-100px)] py-8 px-4 max-w-6xl mx-auto flex flex-col gap-8 animate-in fade-in duration-500">
      ...
      <div className="flex border-b border-card-border/80 p-1 bg-card-bg/20 rounded-xl max-w-fit gap-2 overflow-x-auto">
        <button
          ...
          className={`px-4 sm:px-6 py-2.5 text-sm font-semibold rounded-lg transition-colors whitespace-nowrap ${activeTab === "general" ? "bg-brand-azul-brillante text-white shadow-md" : "text-text-secondary hover:text-text-primary"}`}
        >
          Resumen General
        </button>
        <button
          onClick={() => setActiveTab("experience")}
          className={`px-4 sm:px-6 py-2.5 text-sm font-semibold rounded-lg transition-colors whitespace-nowrap ${activeTab === "experience" ? "bg-brand-azul-brillante text-white shadow-md" : "text-text-secondary hover:text-text-primary"}`}
        >
          Experiencia y Skills
        </button>
        <button
          onClick={() => setActiveTab("projects")}
          className={`px-4 sm:px-6 py-2.5 text-sm font-semibold rounded-lg transition-colors whitespace-nowrap ${activeTab === "projects" ? "bg-brand-azul-brillante text-white shadow-md" : "text-text-secondary hover:text-text-primary"}`}
        >
          Proyectos ({portafolio.proyectos.length})
        </button>
        <button
          onClick={() => setActiveTab("education")}
          className={`px-4 sm:px-6 py-2.5 text-sm font-semibold rounded-lg transition-colors whitespace-nowrap ${activeTab === "education" ? "bg-brand-azul-brillante text-white shadow-md" : "text-text-secondary hover:text-text-primary"}`}
        >
          Educación y Enlaces
        </button>
      </div>
